...
* 7. Participant must give allowance to TokenEcosystemParticipationContract.
* 8. Participant must charge the contract with tokens.
 */

// TODO - bulk_withdraw ??? - the same as withdraw but will accept multiple wallets for the events - GAS
// TODO - events for bulk withdraw with value only ??? - GAS
// TODO - remove unnecesesary events
// ballance will be checket on every withdraw in dapp but the track will be kept in DB !!!
...
contract TokenEcoSystemParticipation is Pausable, Ownable {
    ...
    // Constants
    address private immutable activator;
    ...
    event ChargeContract(
        ...
        uint256 indexed chargedPoints,
        uint256 indexed burnedTokens,
        uint256 indexed chargeDate
    );

    event Activate(address indexed activator, uint256 timestamp);

    event Deactivate(address indexed deactivator, uint256 timestamp);
    ...
    event TotalDailyWithdraws(uint256 indexed timestamp, uint256 value);

    event ModulatorChanged(uint256 newValue);

    event NewAcceptedSender(address indexed newAcceptedSender);

    event GetRate(uint256 rate, uint256 decimals, string rateString, uint256 date, string feed);

    // TODO - emiting such events will be costly when large lenght of arrays!!!
    // can emit total value in (points and or ptrn) and number of addresses!
    // event BulkWithdraw(uint256 indexed timestamp, uint256 value, address[] wallets, uint256[] values);
    ...
    modifier activated() {
        require(acceptedSenderAddress != address(0), "Not activated!");
        _;
    }
    ...
    // Constructor
    constructor(address _acceptedTokens, address _activator) {
        require(_activator != address(0));
    ...
    function activate() public whenNotPaused {
        require(activator == msg.sender, "Activation not allowed!");
        require(acceptedSenderAddress == address(0), "Already activated.");
        acceptedSenderAddress = msg.sender;
        emit Activate(acceptedSenderAddress, block.timestamp);
    }

    // Deactivates the contract by participator
    function deactivate() public activated onlyAuthorized whenNotPaused {
        acceptedSenderAddress = address(0);
        emit Deactivate(msg.sender, block.timestamp);
    }
    ...
    onlyAuthorized
    {
        require(newAcceptedSender != address(0));
        acceptedSenderAddress = newAcceptedSender;
        emit NewAcceptedSender(newAcceptedSender);
    }
    ...
    function setModulator(uint256 value) public whenNotPaused onlyOwner {
        ...
        emit ModulatorChanged(value);
        modulator = value;
    }

    function viewModulator() public view returns (uint256){
        return modulator;
    }
    ...
    function currentRateFeed() public view returns (string memory, address) {
    ...
    function calculateAmount(
        ...
        return ((points * 10 ** decimals) / rate ) * modulator;
    }
    ...
    // The participator charges the contract with Tokens
    ...
    // Returns balance of native tokens which are accidentally sent to the contract not using the chargeContract function
    ...
    function withdraw(uint256 value, bytes32 txHash) public onlyOwner activated {
        require(
        ...
        emit Withdraw(block.timestamp, acceptedSenderAddress, value, txHash);
        pointsBalance -= value;
    }

    // function bulkWithdraw(uint256 value, address[] calldata wallets, uint256[] calldata values) public onlyOwner returns (bool){
    //     require(
    //         value <= pointsBalance, "Not enough points. The contract must be charged with more tokens"
    //     );
    //     require(value > 0, "Withdraw value must be greater than 0!");
    //     emit BulkWithdraw(block.timestamp, value, wallets, values);
    //     pointsBalance -= value;
    //     return true;
    // }
    ...
    function getActivatorAndAcceptedSenderAddress()
    public
    view
    activated
    returns (address, address)
    {
        return (activator, acceptedSenderAddress);
    }
    ...
    function calculatePoints(
        ...
        return ((amount / modulator) * rate) / 10 ** decimals;
    }
    ...
    function burnIncomingTokens(uint256 amount) private returns (bool) {
        ...
        token.burn(amount);
        require(balanceOf() == balance - amount, "transaction reverted burn unsuccesfull!");
        return true;
    }
}

// TODO - remove
// dummy - 0x29c11c0Ea107F5E00Bc71C68fc3598Fa20a2ee9c
// rate pool - 0xeFEABf68B08f94EEc45DC7b64a80f5280DAB7E56
// rate aggr - 0x7ABbC9D596e7e49d2754f9c823f545f8056Feae8
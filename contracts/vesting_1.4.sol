...
contract VestingTeamAndAdvisors is Ownable {
    ...
    */
     ...
     * Approve payment.
     */
    function approvePayment(address beneficiary_, uint256 index) public onlyOwner {
        require(msg.sender == approver);
        require(_beneficiaries[beneficiary_].length > 0, "Beneficiary not existing!");
        _beneficiaries[beneficiary_][index][3] = 1;
    }


    function massApproval(address[] calldata beneficiaries_, uint256[][] calldata indexes_per_beneficiary) public onlyOwner {
        ...
        for (uint256 i = 0; i < beneficiaries_.length; i++) {
            ...
            for (uint256 j = 0; j < indexes_per_beneficiary[i].length; j++) {
                approvePayment(beneficiaries_[i], indexes_per_beneficiary[i][j]);
            }

...
contract VestingTeamAndAdvisors is Ownable {
    ...
    */
     ...
     */
    function beneficiary(address beneficiary_) external view returns (uint256[3][] memory){
    ...
    function addBeneficiary(address beneficiary_, uint256[] calldata amounts, uint256[] calldata releaseDates) external onlyOwner {
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            ...
            _beneficiaries[beneficiary_].push([amounts[i], releaseDates[i], paid]);
        }
     ...
     */
    function cancelPaymentsToBeneficiary(address beneficiary_, string calldata reason) external onlyOwner {
        require(_beneficiaries[beneficiary_].length > 0, "Beneficiary not existing!");
        uint256 j;
        uint256 counter = 0;

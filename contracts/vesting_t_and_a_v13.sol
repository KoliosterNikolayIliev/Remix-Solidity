...
contract VestingTeamAndAdvisors is Ownable {
    ...
    */
    ...
    function getReasonForCancellation(address beneficiary_) external view returns (string[] memory){
        require(msg.sender == owner() || msg.sender == beneficiary_, "Can be viewed only by beneficiary or contract owner!");
        uint256 counter = 0;
    ...
    function _addMultiplePayments(address beneficiary_, uint256[] calldata amounts, uint256[] calldata releaseDates) private {
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            ...
            require(_totalBeneficiariesAmount + totalCurrentBeneficiaryAmount <= _currentBalance(), "Insufficient balance! Can't add new beneficiaries!");
    ...
    //             }
    //         }
    //         require(_beneficiaryList.length - 1 == modifiedList.length);
    ...
    function changeBeneficiaryWallet(address newBeneficiary) external {
        ...
        require(_beneficiaries[newBeneficiary].length <= 0, "Change not possible.");

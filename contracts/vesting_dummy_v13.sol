...
contract V13Dummy is Ownable {
    ...
    */
    ...
    function beneficiary(address beneficiary_) external view returns (uint256[4][] memory){
        require(msg.sender == owner() || msg.sender == beneficiary_, "Caller is not beneficiary!");
    ...
    function getReasonForCancellation(address beneficiary_) external view returns (string[] memory){
        require(msg.sender == owner() || msg.sender == beneficiary_, "Can be viewed only by beneficiary owner!");
        uint256 counter = 0;
        ...
        require(counter > 0, "Does not have canceled payments!");
    ...
    function addBeneficiary(address beneficiary_, uint256[] calldata amounts, uint256[] calldata releaseDates) external onlyOwner {
        require(amounts.length == releaseDates.length && amounts.length >= 1, "Data for not correct!. Count of release dates and amounts must be equal and greater than zero!");
    ...
    function addPaymentToBeneficiary(address beneficiary_, uint256 amount, uint256 releaseDate) external onlyOwner {
        require(_beneficiaries[beneficiary_].length > 0, "Beneficiary not existing!");
        require(amount + _totalBeneficiariesAmount <= _currentBalance(), "Insufficient balance! Can't add new payments!");
    ...
    function cancelPaymentsToBeneficiary(address beneficiary_, string calldata reason, bool forceDelete) external onlyOwner {
        ...
        if (_beneficiaries[beneficiary_].length == 0) {
            ...
            require(_beneficiaryList.length - 1 == modifiedList.length,"aha");
    ...
    function changeBeneficiaryWallet(address newBeneficiary) external {
        require(newBeneficiary != msg.sender, "Change not possible.");
        require(_beneficiaries[msg.sender].length > 0, "Only existing beneficiary can change address!");
        require(_beneficiaries[newBeneficiary].length <= 0, "Change impossible.");
        uint256[4][] memory old =_beneficiaries[msg.sender];
        delete _beneficiaries[msg.sender];
        _beneficiaries[newBeneficiary] = old;

        for (uint256 i = 0; i < _beneficiaryList.length; i++) {

            if (msg.sender == _beneficiaryList[i]) {
    ...
    function release(address beneficiary_) external {
        require(msg.sender == owner() || msg.sender == beneficiary_, "Withdraw can be done only by beneficiary!");
        require(_beneficiaries[beneficiary_].length > 0, "Beneficiary not existing!");
        ...
        require(counter == 0,"No payments for transfer!");
    ...
    function transferExcessBalance(address wallet) external onlyOwner {
        ...
        require(amount > 0, "No excess tokens to transfer!");
    ...
    function massApproval(address[] calldata beneficiaries, uint256[][] calldata indexesPerBeneficiary) public {
        ...
        for (uint256 i = 0; i < beneficiaries.length; i++) {
            require(indexesPerBeneficiary[i].length > 0, "No payments per beneficiary");

...
contract VestingTeamAndAdvisors is Ownable {
    ...
    * TODO 
    * - add approver - done
    * - Add changed wallet mapping/list - done
    * - Add approved to every payment
    * - Delete paymen by index, not in loop
    * - Release payment by index, not in loop
    */
     ...
     */
    constructor(
        Token token_, address approver_
    ) {
        _token = token_;
    ...
    */
    ...
    /**
     * Removes payment from beneficiary.
     */
    function cancelPaymentToBeneficiary(address beneficiary_, uint256 index, string calldata reason) external onlyOwner {
        require(_beneficiaries[beneficiary_].length > 0, "Beneficiary not existing!");
            
        if (timeNow() < _beneficiaries[beneficiary_][index][1] && _beneficiaries[beneficiary_][index][2] == 0 && _beneficiaries[beneficiary_][index][3] == 0) {
            _totalBeneficiariesAmount -= _beneficiaries[beneficiary_][index][0];
            _canceledPaymentsDueToReason[beneficiary_].push(reason);
            _canceledPaymentsAddressList.push(beneficiary_);
            removeFromPaymentList(index,beneficiary_);
        }
        
        else {
            revert("Can't delete approved,withdrawn or after duedate payment!");
        }
    ...
    /**
     * Removes beneficiary from the list if the beneficiary is deleted. !!!Does not keep the order of the array!!!
     */
    ...
    /**
     * Removes payment from beneficiary. !!!Does not keep the order of the array!!!
     */
    function removeFromPaymentList(uint index, address beneficiary_) private {
        _beneficiaries[beneficiary_][index] = _beneficiaries[beneficiary_][_beneficiaries[beneficiary_].length - 1];
    ...
    function checkWalletChange(address wallet) external view onlyOwner returns (address){
        require(_changed_wallet[wallet] != 0x0000000000000000000000000000000000000000, "Wallet never changed!");
